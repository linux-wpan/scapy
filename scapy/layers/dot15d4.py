...
class Dot15d4(Packet):
    ...
    def mysummary(self):
        return self.sprintf("IEEE 802.15.4 %Dot15d4.fcf_frametype% ackreq(%Dot15d4.fcf_ackreq%) ( %Dot15d4.fcf_destaddrmode% -> %Dot15d4.fcf_srcaddrmode% ) Seq#%Dot15d4.seqnum%")
...
class Dot15d4Cmd(Packet):
    ...
    def mysummary(self):
        return self.sprintf("IEEE 802.15.4 Command %Dot15d4Cmd.cmd_id% ( %Dot15dCmd.src_panid%:%Dot15d4Cmd.src_addr% -> %Dot15d4Cmd.dest_panid%:%Dot15d4Cmd.dest_addr% )")
...
class Dot15d4CmdAssocResp(Packet):
    ...
    def mysummary(self):
        return self.sprintf("IEEE 802.15.4 Association Response Payload ( Association Status: %Dot15d4CmdAssocResp.association_status% Assigned Address: %Dot15d4CmdAssocResp.short_address% )")

class Dot15d4CmdDisassociation(Packet):
    name = "IEEE 802.15.4 Disassociation Notification Payload"
    fields_desc = [
